comment on function public.recalculate_profile_stats(uuid) is
'MVP-safe full stat recalculation. Stats are derived only from confirmed matches, and a full recount is acceptable at current scale and safe to retry.';
...
begin
  ...
  -- Trust model: one player submits a result, the other player confirms it before
  -- stats and downstream derived data can change. Safe retries must not double-count.
